...
function editHandler(currentUserDetails) {
    ...
    const user_email_address = document.getElementById('user_email_address');
    ...
    user_phone.value = currentUserDetails.mobile;
    user_email_address.value = currentUserDetails.email;
...
function updateUsers() {
    ...
    if (!user_phone.value) {
        ...
        mobileElementError.innerHTML = 'Enter 10 digit mobile number';
        errorCount = errorCount + 1;
    } else {
    ...
    if(errorCount === 0 ){
        const users = localStorage.getItem('users') ? JSON.parse(localStorage.getItem('users')) : [];
        const loggedInUser = localStorage.getItem('loggedInUser');
        if (!loggedInUser || loggedInUser !== user_email_address.value || users.length <= 0) {
            location.href = './../login/login.html';
            return;
        }
        
        const index = users.findIndex(obj => obj.email ===user_email_address.value);
        if(index === -1) {
            location.href = './../login/login.html';
            return;
        } else {
